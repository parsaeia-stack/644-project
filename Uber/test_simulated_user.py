import random
# Simulated user for testing dialogue systems, this one will provide the correct answer only 50% of the time, and will provide a wrong answer 30% of the time. It will confirm correct values 60% of the time, and reject incorrect values 60% of the time, and the rest is irrelevant noise.
    ...
    def respond(self, system_action, current_values):
        ...
        # --- CONFIRM actions ---
            ...
            if current_values["pickup"] == self.goal["pickup"]:
                return random.choices(["confirm_pos_pickup", "irrelevant"], weights=[60, 40])[0]
            else:
                return random.choices(["confirm_neg_pickup", "irrelevant"], weights=[60, 40])[0]

        elif system_action == "request_confirm_dropoff":
            if current_values["dropoff"] == self.goal["dropoff"]:
                return random.choices(["confirm_pos_dropoff", "irrelevant"], weights=[60, 40])[0]
            else:
                return random.choices(["confirm_neg_dropoff", "irrelevant"], weights=[60, 40])[0]

        elif system_action == "request_confirm_ridetype":
            if current_values["ridetype"] == self.goal["ridetype"]:
                return random.choices(["confirm_pos_ridetype", "irrelevant"], weights=[60, 40])[0]
            else:
                return random.choices(["confirm_neg_ridetype", "irrelevant"], weights=[60, 40])[0]

        elif system_action == "request_confirm_time":
            if current_values["time"] == self.goal["time"]:
                return random.choices(["confirm_pos_time", "irrelevant"], weights=[60, 40])[0]
            else:
                return random.choices(["confirm_neg_time", "irrelevant"], weights=[60, 40])[0]

        elif system_action == "request_confirm_passengers":
            if current_values["passengers"] == self.goal["passengers"]:
                return random.choices(["confirm_pos_passengers", "irrelevant"], weights=[60, 40])[0]
            else:
                return random.choices(["confirm_neg_passengers", "irrelevant"], weights=[60, 40])[0]

        elif system_action == "request_confirm_payment":
            if current_values["payment"] == self.goal["payment"]:
                return random.choices(["confirm_pos_payment", "irrelevant"], weights=[60, 40])[0]
            else:
                return random.choices(["confirm_neg_payment", "irrelevant"], weights=[60, 40])[0]
